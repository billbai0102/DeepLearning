    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            for idx, (data, target) in enumerate(self.dl):
                ...
                # calculate gen loss
                gen_loss = self.dis.loss(dis_out_gen, real_labels)
                gen_loss.backward()
                self.gen_opt.step()
                ...
                dis_loss_real = self.dis.loss(dis_out_real, real_labels)
                
                # train on **fake** data
                dis_out_fake = self.dis(gen_out_fake.detach(), gen_labels)
                dis_loss_fake = self.dis.loss(dis_out_fake, fake_labels)
                ...
                if idx > 0 and idx % (len(self.dl) // 2) == 0:
                    ...
                    # print status
                    print(f'\nEpoch: {epoch + 1}/{EPOCHS} -- Batch: {idx}/{len(self.dl)}'
                          f'\nDiscriminator loss: {dis_loss:.4f}'
                          f'\nGenerator loss: {gen_loss:.4f}'
                          f'\nTime taken: {time.time() - running_time :.3f} seconds')
                    running_time = time.time()
